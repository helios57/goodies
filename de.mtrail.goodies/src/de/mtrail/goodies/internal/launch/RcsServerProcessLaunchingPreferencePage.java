 ...
 * <p>
 * "Field" Komponenten werden mit den Preference-Keys (bzw. Names) initialisiert.
 * <p>
 * Wir speichern die Pfadangaben pauschal unter dem Namen {@link RcsServerArgument#RCS_CONFIG_DIR} ab. Dort befinden sich dann entweder die
 * RCS- oder die RTEX-Default-Pfade.
 */
public class RcsServerProcessLaunchingPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
  ...
  protected void createFieldEditors() {
    ...
    addField(new StringFieldEditor(RcsServerArgument.RCS_WORKING_DIR.name(), "Working directory:", //$NON-NLS-1$
        getFieldEditorParent()));
    addField(new StringFieldEditor(ConfigLocationsVariableResolver.RCS_PROCESS_ENVIRONMENT, "rcs.process.environment:", //$NON-NLS-1$
        getFieldEditorParent()));
    ...
    editor = new EditEnabledListEditor(RcsServerArgument.RCS_CONFIG_DIR.name(), "Configuration path:", configParent, "Configuration",
        "Configuration path entry:");

    addField(editor);

    final Button button = createButton(configParent, String.format("Switch to %s Config", isRcsConfig ? "RTEX" : "RCS"));
  ...
  private static class EditEnabledListEditor extends ListEditor {
    ...
    protected EditEnabledListEditor(final String name, final String labelText, final Composite parent, final String title,
        final String messageBoxText) {
