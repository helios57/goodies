...
public class OpenCloseProjectsJob extends AbstractWorkspaceJobWithSummary {
  ...
  public OpenCloseProjectsJob(final SummaryCollector summaryCollector, final Map<String, BundleConfig> workspaceConfiguration) {
  ...
  private void applyAction(final String id, final List<WorkspaceProject> projects) {
    ...
    // we need to apply a custom selection, but both actions handle these separately.
  ...
  private void findProjectsToCloseAndOpen(final Map<String, BundleConfig> bundleConfigIdx, final Map<String, IProject> projectIdx,
      final List<WorkspaceProject> projectsToClose, final List<WorkspaceProject> projectsToOpen) {
    // Für alle Projekte in den workspace properties entsprechende Aktion merken und die Parameter-Listen füllen
    for (final BundleConfig bundleConfig : bundleConfigIdx.values()) {
      ...
      // Projekte schließen, die explizit als "closed" gekennzeichnet sind, Jene die explizit als "open" gekennzeichnet sind, werden geöffnet.
