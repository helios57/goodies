...
/**
 * This handler launches a bunch of jobs to (pre-)configure a RCS based workspace.
 */
public class WorkspaceConfigurationHandler extends AbstractHandler {
  ...
  private final PreferenceSupport preferenceSupport = new PreferenceSupport(GoodiesPlugin.getDefault().getPreferenceStore());
   ...
   * </pre>
   *
   * All 'real' configuration jobs, like {@link ImportProjectsJob}, {@link OpenCloseProjectsJob} and {@link SortProjectsIntoWorkingSetsJob}
   * are "LONG", while the show summary is "BUILD", to ensure, that this jobs runs definitely only when those other jobs are finished.
   */
  private List<Job> createJobList(final String pathString, final Map<String, BundleConfig> workspaceConfiguration) {
    ...
    if (preferenceSupport.getBoolean(GoodiesPreferenceConstants.IMPORT_PROJECTS)) {
      workspaceConfigJobs.add(new ImportProjectsJob(pathString, workspaceConfiguration, summaryCollector, preferenceSupport));
    }
